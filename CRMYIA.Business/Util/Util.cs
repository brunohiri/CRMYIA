    ...
    public static class Util
    {
        ...
        public static List<CalendarioSazonal> CalcularFeriado(int ano, int QuantidadeAnos)
        {
            ...
            while (i < QuantidadeAnos)
            {
                ...
                ListCalendarioSazonal.Add(new CalendarioSazonal() { Descricao = "Páscoa", Cor = "#1B5E20", Tipo = (byte)EnumeradorModel.TipoSazonal.Feriado, GuidId = GuidIdPascoa.ToString(), DataSazonal = data, DataCadastro = DateTime.Now, DataInicio = data.AddDays(-antes), DataFim = data.AddDays(depois), ExisteCampanha = true, Ativo = true, Repete = (byte)EnumeradorModel.Repete.ACadaAno, Frequencia = (byte)EnumeradorModel.Frequencia.Anualmente });
